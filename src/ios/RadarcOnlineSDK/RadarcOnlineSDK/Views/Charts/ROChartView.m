...
- (void)configureAxes {
    ...
    if (_chartType != ROChartTypePie) {
        ...
        if (_chartType == ROChartTypeLine) {
            axisSet.xAxis.orthogonalCoordinateDecimal = CPTDecimalFromUnsignedInteger(0);
        ...
        for (int i=0; i!=n; i++) {
            ...
            CPTAxisLabel *newLabel = [[CPTAxisLabel alloc]
                                      ...
                                      textStyle:textStyle];
            newLabel.tickLocation = [[self tickLocationAtindex:i] decimalValue];
            newLabel.offset = 2.0;
...
- (void)configureBarChart
{
    ...
    for (ROChartSerie *chartSerie in self.series) {
        ...
        plot.delegate = self;
        plot.barWidth = CPTDecimalFromDouble(_barWidth);
